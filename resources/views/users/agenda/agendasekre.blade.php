    @extends('users.master')
    @section('title', 'Agenda - DPRD')

    @section('content')

    {{-- Agenda section --}}
    <div class="w3l-homeblock2 py-5">
        <div class="w3l-error-grid py-3 text-center">
            <div class="center-align">
                <h2>Agenda Sekretariat</h2>
                <p>Agenda Terbaru Sekretariat DPRD Provinsi Kalimantan.</p>
            </div>
        </div>
        {{-- isi --}}
        <div class="container py-lg-5 py-md-4">
            <!-- block -->
            <div class="row">
                <div class="col-lg-12">
                    <div class="row">
                        <div class="col-lg-3 col-md-4 mt-md-0 mt-sm-5 mt-4 mb-4">
                            <div class="card">
                                <div class="card-header p-0 position-relative">
                                    <a href="#blog-single.html">
                                        <img class="card-img-bottom d-block radius-image-full"
                                            src="{{ asset('assets/images/image2.jpg') }}" alt="Card image cap">
                                    </a>
                                </div>
                                <div class="card-body blog-details">
                                    <a href="#blog-single.html" class="blog-desc">2 New outfit formulas to add to your
                                        Capsule Wardrobe
                                    </a>
                                    <p>Lorem ipsum dolor sit amet consectetur ipsum adipisicing elit. Quis
                                        vitae sit.</p>
                                    <div class="authorag align-items-center mt-3 mb-1">
                                        <ul class="blog-meta">
                                            <li>
                                                <a href="author.html">Charlotte mia</a> </a>
                                            </li>
                                            <li class="meta-item blog-lesson">
                                                <span class="meta-value"> July 13, 2020 </span>. <span
                                                    class="meta-value ml-2"><span class="fa fa-clock-o"></span> 1
                                                    min</span>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- /homeblock1-->
        <section class="w3l-homeblock1 py-sm-1 py-4">
            <div class="container py-md-4">
                <div class="grids-area-hny main-cont-wthree-fea row">
                    <div class="col-lg-6 col-6 grids-feature">
                        <a href="beauty.html">
                            <div class="custom-box">
                                <img src="{{ asset('assets/images/set1.jpeg') }}" alt=""
                                    class="card-body blogcustom-details">
                            </div>
                        </a>
                    </div>
                    <div class="col-lg-6 col-6 grids-feature">
                        <a href="fashion.html">
                            <div class="custom-box">
                                <img src="{{ asset('assets/images/set1.jpeg') }}" alt=""
                                    class="card-body blogcustom-details">
                            </div>
                        </a>
                    </div>
                </div>
            </div>
        </section>
        <!-- //homeblock1-->
        <!-- /homeblock1-->
        <section class="w3l-homeblock1 py-sm-1 py-4">
            <div class="container py-md-4">
                <div class="grids-area-hny main-cont-wthree-fea row">
                    <div class="col-12 grids-feature">
                        <a href="fashion.html">
                            <div class="custom-box">
                                <img src="{{ asset('assets/images/set.jpeg') }}" alt=""
                                    class="card-body blogcustom-details">
                            </div>
                        </a>
                    </div>
                </div>
            </div>
        </section>
        <!-- //homeblock1-->
    </div>

    @endsection
